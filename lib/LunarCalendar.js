...
(function(){
	...
	*/
	 ...
	 * @param {Number} startDay 默认周日（0）开始，最长到周六（6）
	 */
	function calendar(_year,_month,fill, startDay){
		var inputDate = formateDate(_year,_month);
		if(inputDate.error)return inputDate;
		var year = inputDate.year;
		var month = inputDate.month;
		
		var calendarData = solarCalendar(year,month+1,fill, startDay);
	 ...
	 */
	function solarCalendar(_year,_month,fill, startDay){
		var inputDate = formateDate(_year,_month);
		if(inputDate.error)return inputDate;
		var year = inputDate.year;
		var month = inputDate.month;
		...
		//
		startDay = startDay ? startDay : 0;
		var lenCreate = res.firstDay - startDay;
		lenCreate = lenCreate >= 0 ? (res.firstDay - startDay) : (7 + lenCreate)

		if(fill){
			if(res.firstDay !== startDay){ //前补
				var preYear = month-1<0 ? year-1 : year;
				var preMonth = month-1<0 ? 11 : month-1;
				preMonthDays = getSolarMonthDays(preYear,preMonth);
				preMonthData = creatLenArr(preYear,preMonth+1,lenCreate,preMonthDays-lenCreate+1);
